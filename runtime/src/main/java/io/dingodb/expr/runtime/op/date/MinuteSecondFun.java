...
@Operators
abstract class MinuteSecondFun extends UnaryOp {
    public static final String NAME = "MINUTE_SECOND";
    private static final long serialVersionUID = -5554647202194449941L;

    static int extractMinuteSecond(@NonNull Date value, ExprConfig config) {
        ...
        return DateTimeUtils.concatIntegers(new int[] {minute, second});
    }

    static int extractMinuteSecond(@NonNull Time value, ExprConfig config) {
        DingoTimeZoneProcessor processor = config.getProcessor();
    ...
    static int extractMinuteSecond(@NonNull Timestamp value, ExprConfig config) {
        DingoTimeZoneProcessor processor = config.getProcessor();
